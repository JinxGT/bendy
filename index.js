...
bot.on('message', async message => {
  if (message.content.startsWith('be!kick')) {
	  if (message.member.haspermission('KICK_MEMBER')) return message.reply("You don't have permission to use this command!")
...
bot.on('message', async message => {
  if (message.content.startsWith('be!ban')) {
	  if (message.member.haspermission('BAN_MEMBER')) return message.reply("You don't have permission to use this command!")
    // Assuming we mention someone in the message, this will return the user
    // Read more about mentions over at https://discord.js.org/#/docs/main/master/class/MessageMentions
    const user = message.mentions.users.first();
    // If we have a user mentioned
